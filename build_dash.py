...
/* Mobile navigation: tabs scroll in one row instead of wrapping. Mirrored in
   the web app's roundbook.css - keep the two in step. */
@media(max-width:700px){
  .tabs{flex-wrap:nowrap;overflow-x:auto;-webkit-overflow-scrolling:touch;scrollbar-width:none;margin-left:-20px;margin-right:-20px;padding:0 20px}
  .tabs::-webkit-scrollbar{display:none}
  .tab{white-space:nowrap;flex:0 0 auto;padding:12px 13px}
  .rangewrap input[type=range]{width:110px}
}

/* App-like navigation: sticky tab bar, mobile scroll hint, dark scrollbars on
   the scorecard. Mirrored in the web app's roundbook.css - keep in step. */
.tabs{position:sticky;top:0;z-index:40;background:var(--bg)}
.scorecard{scrollbar-width:thin;scrollbar-color:#35493b transparent}
.scorecard::-webkit-scrollbar{height:8px}
.scorecard::-webkit-scrollbar-thumb{background:#35493b;border-radius:4px}
.scorecard::-webkit-scrollbar-track{background:transparent}
@media(max-width:700px){
...
function renderApproach(){
  ...
  document.getElementById('hvHead').textContent=!HVok.length?'Not enough approach shots in these windows to compare.'
  ...
  document.getElementById('hvSub').innerHTML=HVW.map(w=>w.st
...
// only builds DOM and the hero chart from the model; the global filter decides
// which rounds are in view. trSel is the metric shown in the hero, tiles and
// power line.
let trSel='ou18', trModel=null;
...
function renderTrends(){
  const t=trendModel(filtered(),D.hcp);trModel=t;
  ...
  if(hasWin){win.style.display='';win.textContent=`Newer half: the last ${t.Rw} rounds (${t.hR} holes). Older half: the ${t.Pw} before them (${t.hP} holes). Change the course, date or last-rounds filter to move the window.`;}
  ...
  if(hasWin){
    cap.textContent=`Every metric, newer half vs older half of the ${N} rounds in view, ranked by how clearly it moved.`;
    ...
    list.innerHTML=rows.map(({x})=>{const v=x.v,m=x.m,tg=TAG[v.state];
      const num=v.state==='early'
        ?`<b>${v.now!=null&&v.nR>=m.dotMin?m.fmt(v.now):'–'}</b> <span>· needs ${[v.needP?`${v.needP} more older-half ${tUnit(v.needP,m.unitW)}`:'',v.needR?`${v.needR} more newer-half ${tUnit(v.needR,m.unitW)}`:''].filter(Boolean).join(', ')}</span>`
...
function renderTrendSel(){
  ...
  if(hasWin&&v.state!=='early'){
    ...
    tiles.innerHTML=trTile(m.fmt(v.before),'older half',`${t.Pw} rounds · ${v.nP} ${tUnit(v.nP,m.unitW)}`)+trTile(m.fmt(v.now),'newer half',`${t.Rw} rounds · ${v.nR} ${tUnit(v.nR,m.unitW)}`)+trTile(m.dfmt(v.delta),'change',sub,v.state==='improving'?'good':v.state==='slipping'?'leak':'');
  } else if(hasWin){
    const need=[v.needP?`${v.needP} more older-half ${tUnit(v.needP,m.unitW)}`:'',v.needR?`${v.needR} more newer-half ${tUnit(v.needR,m.unitW)}`:''].filter(Boolean).join(', ');
    tiles.innerHTML=trTile(v.before!=null&&v.nP>=m.dotMin?m.fmt(v.before):'–','older half',`${t.Pw} rounds · ${v.nP} ${tUnit(v.nP,m.unitW)}`)+trTile(v.now!=null&&v.nR>=m.dotMin?m.fmt(v.now):'–','newer half',`${t.Rw} rounds · ${v.nR} ${tUnit(v.nR,m.unitW)}`)+trTile('–','change',`needs ${need}`);
  } else {
    tiles.innerHTML=trTile(x.allN>=m.dotMin?m.fmt(x.allV):'–','in view',`${N} round${N===1?'':'s'} · ${x.allN} ${tUnit(x.allN,m.unitW)}`)+trTile(N,'rounds in view',`a comparison needs ${TREND_MIN_ROUNDS}`)+trTile('–','change','widen the filter');
  }
  ...
  if(hasWin){
    ...
    else pw.textContent=`To call a change of ${eff} in ${lab} at your current spread takes ${x.need>1000?'more than 1000':'about '+x.need} ${m.unitW} on each side. You have ${v.nR} in the newer half and ${v.nP} in the older`+(v.state==='early'?`; a call also needs ${m.gate} on each side.`:'.');
  } else pw.textContent='';
}
...
<div class="wrap">
  ...
  <section class="panel" id="p-approach">
    ...
    <div class="card full" style="margin-top:16px;border-color:rgba(201,162,74,.3)">
      <p class="eyebrow">Against your handicap</p>
      <h4 id="hvHead" style="font-size:18px;margin:0 0 4px"></h4>
      <p class="cap" id="hvSub" style="margin:0 0 12px"></p>
      <div class="grid">
        <div style="min-width:0"><h4 style="font-size:15px">60 to 80 yards</h4><div class="chartbox" style="height:210px"><canvas id="hv1"></canvas></div></div>
        <div style="min-width:0"><h4 style="font-size:15px">100 to 120 yards</h4><div class="chartbox" style="height:210px"><canvas id="hv2"></canvas></div></div>
      </div>
      <p class="note">Handicap bars are Arccos's published average distance to the pin on approach shots, by handicap bracket, from its players' shots. Arccos publishes them for these two windows only, so the other distances have no handicap reference. Your bar is the average of your approach shots from the same window, by the same approach-only rule as the charts above, with the shot count in brackets. These are averages, not medians, so one bad miss moves yours.</p>
    </div>
    <div class="card full" style="margin-top:16px">
  ...
  <section class="panel" id="p-trends">
    <div class="card full" style="margin-bottom:16px;border-color:rgba(201,162,74,.3)">
      ...
      <p class="cap" id="trSub" style="margin:0"></p>
