...
- Approach tab distance bands (hit-green % and proximity vs TOURPROX) count approaches only (Sep 2026): the payload's ap holds every shot from 50y+, so the engine drops the par-4/5 drive (first ap entry on a hole with no penalty) and anything starting beyond reach (longest non-driver D.ladder smart distance + 20y). Before this, drives and layups made the 200+ band read ~350 ft. Bands under 5 shots are not drawn; the note under the charts names them.
