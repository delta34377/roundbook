...
function fetchProfile(uid: string, token: string): Promise<any> {
...
function pickIndex(obj: any, depth = 0): { value: number; plus: boolean } | null {
...
async function fetchGhinIndex(email: string, password: string): Promise<{ index: number; source: string } | { error: string }> {
